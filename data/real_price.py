...
async def get_items_by_collection(
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(
            f"https://api.rarible.org/v0.1/items/byCollection?collection={collection}&size=10000"
        ) as response:
...
async def get_traits(
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(
            f"https://api.rarible.org/v0.1/items/traits?collectionIds={collection}&keys={','.join(traits_list)}"
        ) as response:
    ...
    for trait in response["traits"]:
        for value in trait["values"]:
            try:
                traits[trait["key"]["value"]].append(
                    {"value": value["value"], "price": 0, "count": value["count"]}
                )
            except KeyError:
...
async def get_item_floor_price(
    item_id: int, collection: str = "POLYGON:0x15f4272460062b835ba0abbf7a5e407f3ef425d3"
):
    item_id = collection + ":" + str(item_id)
    async with aiohttp.ClientSession() as session:
        async with session.get(
            f"https://api.rarible.org/v0.1/items/{item_id}"
        ) as response:
            response = await response.json()
    try:
        ...
        return {
            "price": floor_price["currentValue"],
            "largest_trait": None,
            "traits": [],
        }
